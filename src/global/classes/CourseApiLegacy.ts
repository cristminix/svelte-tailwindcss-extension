...
import { getDsRecordsByType } from "../fn/course/legacy/getDsRecordsByType"
import type { CourseInfoLegacyInterface } from "./types"
import { slugify } from "../fn/course/slugify"
import { courseUrlFromSlug } from "../fn/course/courseUrlFromSlug"
import { slugToTitle } from "../fn/course/slugToTitle"

export class CourseApiLegacy {
  // store m3Rec
  ds: any = null
  course: any = null
  sections: any = null
  ...
  authors: any = null
  slug: any = null
  exerciseFiles: any = null
  thumbnails: any = null
  store: any = null
  constructor(ds: any, slug: string, store: any) {
    this.ds = ds
    this.slug = slug
    this.store = store
  }
  ...
  async getCourse(refresh = false) {
    ...
    if (rows.length > 0) {
      ...
      if (filteredRows.length > 0) {
        const row = filteredRows.pop() as any
        console.log(row)
        ...
        if (row.primaryThumbnail) {
          const rootUrl = findPropLegacy("rootUrl", row.primaryThumbnail, 1)
          const artifacts = findPropLegacy("artifacts", row.primaryThumbnail, 1)
          ...
          const rootUrl = findPropLegacy("rootUrl", row.primaryThumbnailV2, 1)
          const artifacts = findPropLegacy("artifacts", row.primaryThumbnailV2, 1)
  ...
  async getSections(refresh = false) {
    ...
    let lac_key = urn
    let lac = findPropLegacy(lac_key, this.ds)
  ...
  async getTocs(refresh = false) {
    ...
    if (!refresh) {
      if (this.tocs) {
        if (this.tocs.length > 0) return this.tocs
      }
    }
    this.tocs = null
    if (this.sections) {
      for (const section of this.sections) {
        ...
        if (!tocsFoundInDb) {
          for (const itemStar of section.itemStars) {
            let videoDataFound = 0
            let videoData = null
            let tocData = null

            let tocDataFound: boolean | number = false
  ...
  async getAuthors(refresh = false) {
    ...
    if (results) {
      for (const _authorData of results) {
        const authorData = _authorData as any
